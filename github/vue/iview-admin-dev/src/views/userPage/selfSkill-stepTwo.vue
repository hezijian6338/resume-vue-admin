...
<div class="skills">
  <template>
    <Form ref="studentResume" :model="studentResume" :rules="ruleValidate" :label-width="80">
        <FormItem label="Skill-Two-Name" prop="skilltwo">
            <Input v-model="studentResume.skilltwo" placeholder="Enter your skill name" style="width: 200px"></Input>
        </FormItem>
        <!-- <FormItem label="Skill-Status" prop="oneStatus">
          <Slider v-model="studentResume.status" :tip-format="format" show-input></Slider>
        </FormItem> -->
        <FormItem label="Skill-Decs" prop="twoinfo">
            <Input v-model="studentResume.twoinfo" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="Enter something..."></Input>
        </FormItem>
        <FormItem>
...
export default {
  data() {
    return {
      ...
      ruleValidate: {
        skilltwo: [{
          required: true,
          message: 'The skilltwo cannot be empty',
          trigger: 'blur'
        }],
        status: [{
          required: true,
        ...
        twoinfo: [{
            required: true,
            message: 'Please enter a twoinfo introduction',
            trigger: 'blur'
          },
          {
            type: 'string',
  ...
  beforeRouteLeave(to, from, next) {
    if (to.path == '/step/index/three') {
      this.handleSubmit('studentResume')
      next()
    } else if (to.path == '/step/index/one') {
      next()
    }
  }
}
